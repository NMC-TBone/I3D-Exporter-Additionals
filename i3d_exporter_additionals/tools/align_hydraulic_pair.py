...
def _aim_rot_preserve_roll(from_loc: Vector, to_loc: Vector, y_sign: float, old_R_world_3x3: Matrix) -> Matrix:
    """World rotation so local Y (±) aims at target, preserving roll from old X. Returns 3x3 (columns=basis)."""
    fwd = (to_loc - from_loc)
    if fwd.length_squared == 0.0:
        return old_R_world_3x3.copy()

    Y = (fwd if y_sign > 0.0 else -fwd).normalized()

    # preserve roll using old world +X
    X_old = old_R_world_3x3.col[0]
    X_proj = X_old - Y * X_old.dot(Y)
    if X_proj.length_squared < 1e-12:
        X_proj = Y.cross(Vector((0, 0, 1)))  # fallback to Y × up(+Z)
        if X_proj.length_squared < 1e-12:
            X_proj = Vector((1, 0, 0))
    X = X_proj.normalized()
...
def _apply_rotation_keep_mesh(
        obj: bpy.types.Object, R_new_3x3: Matrix,
        loc_snapshot: Vector, R_old_3x3_snapshot: Matrix, scale_snapshot: Vector) -> None:
    ...
    S4 = Matrix.Diagonal((scale_snapshot.x, scale_snapshot.y, scale_snapshot.z, 1.0))
    R_old4 = R_old_3x3_snapshot.to_4x4()
    R_new4 = R_new_3x3.to_4x4()

    # Exact local-space correction that preserves world-space shape:
    # (works for non-uniform scales; may introduce local shear, which cancels with S4)
    M_mesh = S4.inverted_safe() @ R_new4.inverted_safe() @ R_old4 @ S4
    obj.data.transform(M_mesh)
    obj.data.update()

    # Build the desired world matrix from the same snapshots
    T = Matrix.Translation(loc_snapshot)
    M_world_new = T @ R_new4 @ S4

    # Let Blender resolve locals (plays nicely with parenting)
    obj.matrix_world = M_world_new
...
class I3DEA_OT_align_hydraulic_pair(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        rot_punch_new = _aim_rot_preserve_roll(
            from_loc=p_loc, to_loc=h_loc, y_sign=+1.0, old_R_world_3x3=rot_punch_old
        )
