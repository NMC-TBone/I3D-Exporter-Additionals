...
class I3DEA_PG_List(bpy.types.PropertyGroup):
    ...
    diffuse_box: BoolProperty(
        name="Add diffuse node", description="If checked it will create a image texture linked to Base Color", default=False)
    ...
    shader_path: StringProperty(name="Path to shader location",
                                description="Select path to the shader you want to apply", subtype='FILE_PATH', default="")
    mask_map: StringProperty(name="Mask Map", description="Add mask map texture", subtype='FILE_PATH', default="")
    dirt_diffuse: StringProperty(name="Dirt diffuse", description="Add dirt diffuse texture",
                                 subtype='FILE_PATH', default="")
    shader_box: BoolProperty(name="Set shader path",
                             description="If checked it will add the the path to the shader in material", default=True)
    mask_map_box: BoolProperty(name="Set mask map path",
                               description="If checked it will add the the path to mask map in material", default=True)
    dirt_diffuse_box: BoolProperty(name="Set dirt diffuse path",
                                   description="If checked it add the the path to dirt diffuse in material", default=True)
    ...
    custom_text_box: BoolProperty(
        name="Custom name", description="If checked you will be able to add custom name for the track pieces", default=False)
    ...
    piece_distance: FloatProperty(name="Track piece distance: ",
                                  description="Add track piece distance", default=0.2, precision=10, min=0.0001)
    curve_length_disp: StringProperty(name="curve_length", default="0.0")
    track_piece_amount: FloatProperty(name="Track pieces possible along curve",
                                      description="The amount of track links that will fit along the curve", min=1, max=400, default=1)
    ...
    add_empties: BoolProperty(
        name="Add Empties", description="If you check this it will add the amount of empties between each track link that's written bellow.", default=False)
    ...
    track_vis_amount: IntProperty(
        ...
        description="Amount of track pieces to use along the curve",
        default=1,
        min=1,
        max=200
    )
    ...
    auto_add_vmask: BoolProperty(
        name="Add Vmask Objects",
        description="Adds pieces ready for AO bake",
        default=False
    )

    auto_fixed_amount: BoolProperty(
        name="Fixed Amount",
        description="If checked you will need to set the amount manually",
        default=False
    )
    ...
    auto_add_empties: BoolProperty(
        name="Empty amount",
        description="If checked it will add the amount of empties bellow",
        default=False
    )
    ...
    auto_create_bbox: BoolProperty(
        name="Add BoundingVolume",
        description="Creates a BV around track",
        default=False
    )

    auto_name: StringProperty(
        name="Custom name",
        description="Name of the track setup",
        default="myTrack"
    )

    auto_distance: FloatProperty(
        name="Distance between links",
        description="Distance between each link",
        default=0.2
    )
    ...
    motion_type: EnumProperty(name="Motion Types",
                              items=[('AMOUNT_REL', 'Fixed Amount', "Places empties on every curve in equal distances per curve"),
                                     ('AMOUNT_FIX', 'Distance from Amount',
                                      "Places empties on longest curve in equal distance. Apply this distance on other curves",),
                                     ('DISTANCE', 'Fixed Distance', "Places Objects in fixed equal distance")])
    motion_amount_rel: IntProperty(
        name="AmountRel",
        default=32,
        min=1,
    )

    motion_amount_fix: IntProperty(
        name="AmountFix",
        default=32,
        min=1
    )
    ...
    motion_hierarchy_name: StringProperty(
        name="Array Name",
        default="curveArray"
    )
